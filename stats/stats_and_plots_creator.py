...
def save_distributions_and_plots(dataset: Dataset,
    ...
    labels = get_labels(dataset, df, taxonomies_and_neighbours)
    save_plot_tsne(labels, df, plot_filename)
    Distribution.save_distributions(labels, stats_filename)


def get_labels(dataset: Dataset, df: pd.DataFrame, taxonomies_and_neighbours: List[TaxonomyAndNeighbours]) -> List[Label]:
    labels = []
    ...
    for i in range(len(taxonomies_and_neighbours)):
        ...
        labels.append(Label(target_class, sensitive_class_values, taxonomy.value))
...
def save_plot_tsne(labels: List, df: pd.DataFrame, plot_filename: str):
    ...
    tsne = TSNE(n_components=2, verbose=2, perplexity=40, n_iter=250, learning_rate=500)
